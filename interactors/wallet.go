...
func (w *wallet) GetPrivateKeyFromSeed(seed []byte, account, addressIndex uint32) []byte {
	var rewaPath = bip32Path{
		44 | hardened,
		rewaCoinType | hardened,
		hardened, // account
		hardened,
		hardened, // addressIndex
	}

	rewaPath[2] = account | hardened
	rewaPath[4] = addressIndex | hardened
	keyData := derivePrivateKey(seed, rewaPath)
