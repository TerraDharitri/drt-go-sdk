...
func TestNewPriceFetcher(t *testing.T) {
	...
	t.Run("nil graphqlGetter should error", func(t *testing.T) {
		...
		args := createMockArgsPriceFetcher()
		args.FetcherName = DharitriXName
		args.GraphqlGetter = nil
		pf, err := NewPriceFetcher(args)
		assert.Nil(t, pf)
		assert.True(t, errors.Is(err, errNilGraphqlGetter))
	})
	t.Run("nil map for DharitriX should error", func(t *testing.T) {
		t.Parallel()

		args := createMockArgsPriceFetcher()
		args.FetcherName = DharitriXName
		args.DharitriXTokensMap = nil
		pf, err := NewPriceFetcher(args)
		assert.Nil(t, pf)
		assert.True(t, errors.Is(err, errNilDharitriXTokensMap))
	})
	t.Run("should work", func(t *testing.T) {
		...
		t.Run("DharitriX", func(t *testing.T) {
			t.Parallel()

			args := createMockArgsPriceFetcher()
			args.FetcherName = DharitriXName
			pf, err := NewPriceFetcher(args)
			assert.Equal(t, "*fetchers.DharitriX", fmt.Sprintf("%T", pf))
			assert.Nil(t, err)
		})
