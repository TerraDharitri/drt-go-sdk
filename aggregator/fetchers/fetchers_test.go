...
func createMockMap() map[string]DharitriXTokensPair {
	return map[string]DharitriXTokensPair{
		"ETH-USD": {
			Base:  "WETH-b4ca29",
			Quote: "USDC-c76f1f",
		},
	...
	for name := range ImplementedFetchers {
		go func(fetcherName string) {
			args := ArgsPriceFetcher{
				...
				DharitriXTokensMap: createMockMap(),
...
func Test_FetchPriceErrors(t *testing.T) {
	...
	for f := range ImplementedFetchers {
		...
		t.Run("DharitriX: missing key from map should error "+fetcherName, func(t *testing.T) {
			t.Parallel()

			if fetcherName != DharitriXName {
				return
			}
		...
		t.Run("DharitriX: invalid graphql response should error "+fetcherName, func(t *testing.T) {
			t.Parallel()
