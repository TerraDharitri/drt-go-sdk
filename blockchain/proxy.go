...
import (
	...
	"github.com/TerraDharitri/drt-go-sdk/blockchain/factory"
...
// ArgsProxy is the DTO used in the dharitri proxy constructor
...
// proxy implements basic functions for interacting with a dharitri Proxy
type proxy struct {
	*baseProxy
	sameScState            bool
	shouldBeSynced         bool
...
// GetDCDTTokenData returns the address' fungible token data
func (ep *proxy) GetDCDTTokenData(
	ctx context.Context,
	address sdkCore.AddressHandler,
	tokenIdentifier string,
	queryOptions api.AccountQueryOptions, // TODO: provide AccountQueryOptions on all accounts-related getters
) (*data.DCDTFungibleTokenData, error) {
	if check.IfNil(address) {
		return nil, ErrNilAddress
	}
	if !address.IsValid() {
	...
	endpoint := ep.endpointProvider.GetDCDTTokenData(addressAsBech32String, tokenIdentifier)
...
// GetNFTTokenData returns the address' NFT/SFT/MetaDCDT token data
